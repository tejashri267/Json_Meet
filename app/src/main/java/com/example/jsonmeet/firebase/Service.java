package com.example.jsonmeet.firebase;
...
import com.example.jsonmeet.activities.IncomigInvitation;
    ...
    public void onMessageReceived(@NonNull RemoteMessage remoteMessage) {
        ...
        if (type != null) {
            if (type.equals(Constants.REMOTE_INVITATION_)) {
                Intent intent = new Intent(getApplicationContext(), IncomigInvitation.class);
                intent.putExtra(
...
//                );
                intent.putExtra(
                        Constants.MEETING_ROOM,
                        remoteMessage.getData().get(Constants.MEETING_ROOM)
                );
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK );
                startActivity(intent);
            } else if (type.equals(Constants.REMOTE_INVITATION_RESPONSE)) {
                Intent intent = new Intent(Constants.REMOTE_INVITATION_RESPONSE);
                intent.putExtra(
                        Constants.REMOTE_INVITATION_RESPONSE,
                        remoteMessage.getData().get(Constants.REMOTE_INVITATION_RESPONSE)
                );
